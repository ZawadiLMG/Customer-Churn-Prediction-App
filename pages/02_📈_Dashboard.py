...
def eda_dashboard():
    ...
    with col3:
        ...
        totalcharges_histogram.update_traces(marker_color="red")
    ...
    with col5:
        churn_gender_counts = final.groupby(['gender', 'churn']).size().reset_index(name='count')
        fig = px.bar(churn_gender_counts, x='gender', y='count', color='churn', barmode='group',
                     color_discrete_map={'No': 'blue', 'Yes': 'red'})
        fig.update_layout(
            xaxis_title='Gender',
            yaxis_title='Count',
            title='Distribution of Churn in Gender'
        )
        st.plotly_chart(fig)
    ...
    with col6:
        fig = px.histogram(final, "contract", color="churn", title="Histogram for contract in churn",
                           color_discrete_map={'Yes': 'red', 'No': 'blue'})
        st.plotly_chart(fig)

    with col7:
        fig = px.histogram(final, "paymentmethod", color="churn", title="Histogram for contract in churn",
                           color_discrete_map={'Yes': 'red', 'No': 'blue'})
        st.plotly_chart(fig)
...
def kpi_dashboard():
    ...
    with col10:
        churn_security_counts = final.groupby(['onlinesecurity', 'churn']).size().reset_index(name='count')
        fig = px.bar(churn_security_counts, x='onlinesecurity', y='count', color='churn', barmode='group',
                     color_discrete_map={'No': 'blue', 'Yes': 'red'})
        fig.update_layout(
            xaxis_title='Online Security',
            yaxis_title='Count',
            title='Distribution of Churn with Online Security'
        )
        st.plotly_chart(fig)

    with col11:
        churn_backup_counts = final.groupby(['onlinebackup', 'churn']).size().reset_index(name='count')
        fig = px.bar(churn_backup_counts, x='onlinebackup', y='count', color='churn', barmode='group',
                     color_discrete_map={'No': 'blue', 'Yes': 'red'})
        fig.update_layout(
            xaxis_title='Online Backup',
            yaxis_title='Count',
            title='Distribution of Churn with Online Backup'
        )
        st.plotly_chart(fig)
    ...
    with col12:
        churn_streamingtv_counts = final.groupby(['streamingtv', 'churn']).size().reset_index(name='count')
        fig = px.bar(churn_streamingtv_counts, x='streamingtv', y='count', color='churn', barmode='group',
                     color_discrete_map={'No': 'blue', 'Yes': 'red'})
        fig.update_layout(
            xaxis_title='Streaming TV',
            yaxis_title='Count',
            title='Distribution of Churn with Streaming TV'
        )
        st.plotly_chart(fig)

    with col13:
        churn_streamingmovies_counts = final.groupby(['streamingmovies', 'churn']).size().reset_index(name='count')
        fig = px.bar(churn_streamingmovies_counts, x='streamingmovies', y='count', color='churn', barmode='group',
                     color_discrete_map={'No': 'blue', 'Yes': 'red'})
        fig.update_layout(
            xaxis_title='Streaming Movies',
            yaxis_title='Count',
            title='Distribution of Churn with Streaming Movies'
        )
        st.plotly_chart(fig)
